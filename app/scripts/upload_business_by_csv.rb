...
CSV.foreach("#{filename}", headers: true) do |row|
  ...
  timestamp          = row[0]
  business_name      = row[1].strip if row[1].present?
  category           = row[2]
  instagram_username = row[3].strip if row[3].present?
  link_tokopedia     = row[4].strip if row[4].present?
  link_bukalapak     = row[5].strip if row[5].present?
  link_shopee        = row[6].strip if row[6].present?
  wa_number          = row[7].strip if row[7].present?
  link_gojek         = row[8].strip if row[8].present?
  link_grab          = row[9].strip if row[9].present?
  others             = row[10]
  location           = row[11].strip if row[11].present?
  description        = row[12].strip if row[12].present?
  assets_urls        = row[13]
